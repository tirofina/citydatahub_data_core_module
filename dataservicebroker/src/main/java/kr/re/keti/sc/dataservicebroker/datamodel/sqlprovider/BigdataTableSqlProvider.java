...
public class BigdataTableSqlProvider {
	...
	public String generateCreateTableDdl(DataModelCacheVO dataModelCacheVO) throws BadRequestException {
		...
		if(!isValidIndexName(dataModelCacheVO.getDataModelVO(), dataModelCacheVO.getDataModelVO().getIndexAttributeNames())) {
			throw new BadRequestException(ErrorCode.INVALID_DATAMODEL, "Invalid IndexAttributeName. indexAttributeNames=" + dataModelCacheVO.getDataModelVO().getIndexAttributeNames());
		}
		...
		// 1. 최종값 테이블 관련 쿼리 생성
		String entityTableName = dataModelManager.generateHiveTableName(dataModelCacheVO.getDataModelVO().getId());
...
//				.append("DATASET_ID VARCHAR(256) NOT NULL ").append(LINE_SEPARATOR).append(COMMA_WITH_SPACE)
		...
		*/
		...
		String entityHistTableName = dataModelManager.generateHiveTableName(dataModelCacheVO.getDataModelVO().getId(), Constants.PARTIAL_HIST_TABLE_PREFIX);
		...
		*/
		...
		String entityFullTableName = dataModelManager.generateHiveTableName(dataModelCacheVO.getDataModelVO().getId(), Constants.FULL_HIST_TABLE_PREFIX);
...
//		sql.append(";").append(LINE_SEPARATOR).append(LINE_SEPARATOR);
	 ...
	 * @return 생성된 sql문
	 */
	private String generateDynamicIndexSql(String dbTableName, List<String> indexAttributeIdList,
		...
		if (indexAttributeIdList != null) {
			for (int i = 0; i < indexAttributeIdList.size(); i++) {
				...
				// sql.append(generateDynamicIndexSql(dbTableName, indexAttributeIdList.get(i) +
				// Constants.GEO_PREFIX_3857));
				// sql.append(generateDynamicIndexSql(dbTableName, indexAttributeIdList.get(i) +
				// Constants.GEO_PREFIX_4326));
				// isGeoType = true;
				// break;
				// }
				// }
				...
				// // geoType이 아닌 경우
				// if(!isGeoType) {
				// sql.append(generateDynamicIndexSql(dbTableName,
				// indexAttributeIdList.get(i)));
				// }
			}
		}
		return sql.toString();
